...
import { EmploymentItem } from "@/types/resume.types";
import { Pencil, PlusCircle, X } from "lucide-react";
import { useState } from "react";

export const EmploymentHistory = () => {
    const [employment, setEmployment] = useState<EmploymentItem[]>([]);
    const [form, setForm] = useState<EmploymentItem>({
        id: Date.now(),
        jobTitle: "",
        company: "",
        startDate: "",
    ...
    // 🔹 Handle form change
    const handleChange = (field: keyof EmploymentItem, value: string) => {
    ...
    const handleAdd = () => {
        ...
        setEmployment((prev) => [...prev, { ...form, id: Date.now() }]);
        resetForm();
    };
    ...
    const handleDelete = (id: number) => {
        setEmployment((prev) => prev.filter((item) => item.id !== id));
    };
